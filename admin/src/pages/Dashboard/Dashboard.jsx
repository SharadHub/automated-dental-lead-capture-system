...
import { MdPerson, MdGpsFixed, MdFavorite, MdCalendarToday } from 'react-icons/md';
...
export default function Dashboard() {
  ...
  useEffect(() => {
    Promise.all([analyticsApi.getOverview(), analyticsApi.getRecentActivity()])
      .then(([ov, rc]) => { setOverview(ov.data); setRecent(rc.data); })
  ...
  const stats = [
    { label: 'Prospects',    value: overview?.prospects    || 0, icon: MdPerson,       color: 'bg-blue-50 text-blue-600',    link: '/leads?status=prospect'  },
    { label: 'Leads',        value: overview?.leads        || 0, icon: MdGpsFixed,     color: 'bg-green-50 text-green-600',  link: '/leads?status=lead'      },
    { label: 'Patients',     value: overview?.patients     || 0, icon: MdFavorite,     color: 'bg-purple-50 text-purple-600', link: '/leads?status=patient'  },
    { label: 'Appointments', value: overview?.appointments || 0, icon: MdCalendarToday, color: 'bg-yellow-50 text-yellow-600', link: '/appointments'          },
  ];
      ...
      <div className="grid lg:grid-cols-3 gap-4">
        <div className="lg:col-span-2 bg-white rounded-xl p-6 border border-gray-100">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link to="/analytics" className="text-sm text-primary-500 hover:underline">View full analytics →</Link>
          </div>
        ...
        <div className="bg-gradient-to-br from-primary-500 to-teal-500 rounded-xl p-6 text-white">
          ...
          <Link to="/appointments" className="bg-white/20 hover:bg-white/30 text-white text-sm px-4 py-2 rounded-lg inline-block transition-all">
            View Schedule →
          </Link>
        </div>
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-xl border border-gray-100">
          ...
          <div className="divide-y divide-gray-50">
            ...
            {!recent?.recentProspects?.length && <p className="p-6 text-center text-gray-400 text-sm">No prospects yet</p>}
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl border border-gray-100">
          <div className="p-4 border-b border-gray-100 flex items-center justify-between">
            ...
            <Link to="/appointments" className="text-xs text-primary-500">View all →</Link>
          </div>
          <div className="divide-y divide-gray-50">
            ...
            {!recent?.recentAppointments?.length && <p className="p-6 text-center text-gray-400 text-sm">No appointments yet</p>}
          </div>
        </div>
      </div>
    </div>
