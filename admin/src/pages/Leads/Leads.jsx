...
export default function Leads() {
  ...
  const [filters, setFilters] = useState({ status: searchParams.get('status') || '', source: '', search: '', page: 1 });
        ...
        <div className="flex-1 bg-white rounded-xl border border-gray-100 overflow-hidden">
          {loading ? (
            ...
            <>
                ...
                <tbody className="divide-y divide-gray-50">
                  {data.data.map(p => (
                    <tr key={p.id} className="hover:bg-gray-50 cursor-pointer" onClick={() => setSelected(p)}>
                      ...
                      <td className="px-4 py-3 text-sm text-gray-600">{SOURCE_LABELS[p.source] || p.source}</td>
                      ...
                      <td className="px-4 py-3" onClick={e => e.stopPropagation()}>
                        <div className="flex gap-1">
                          {p.phone && (
                            <a href={`tel:${p.phone}`} className="p-1.5 hover:bg-green-50 rounded-lg text-gray-400 hover:text-green-600 transition-colors" title="Call">
                              <FiPhone size={14} />
                            </a>
                          )}
                          {p.status === 'prospect' && (
                            <button onClick={() => updateStatus(p.id, 'lead')} className="text-xs bg-blue-50 text-blue-600 px-2 py-1 rounded hover:bg-blue-100">
                              → Lead
                            </button>
                          )}
                        </div>
              ...
              {!data.data.length && <p className="text-center text-gray-400 py-12">No prospects found</p>}
            </>
          )}
        </div>
        ...
        {selected && (
          <div className="w-72 bg-white rounded-xl border border-gray-100 p-5 flex-shrink-0">
            ...
            <div className="space-y-2 text-sm mb-4">
              ...
              <div className="flex gap-2"><span className="text-gray-400">Source:</span><span className="text-gray-700">{SOURCE_LABELS[selected.source] || selected.source}</span></div>
              ...
              {selected.message && (
                <div><span className="text-gray-400">Message:</span><p className="text-gray-700 mt-1 bg-gray-50 rounded p-2 text-xs">{selected.message}</p></div>
              )}
            </div>
            ...
            {selected.phone && (
              <a href={`tel:${selected.phone}`} className="flex items-center justify-center gap-2 bg-green-500 hover:bg-green-600 text-white rounded-lg py-2 mt-4 text-sm font-medium transition-colors">
